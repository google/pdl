...
use super::{import, Class, PacketDef, Type, Variable};
use crate::{ast::EndiannessValue, backends::common::alignment::Chunk};

impl FormatInto<Java> for &Class {
    fn format_into(self, tokens: &mut Tokens<Java>) {
        quote_in!(*tokens => $(match self {
            Class::Packet(def) => $def,
            _ => todo!()
        }));
    }
}

impl FormatInto<Java> for &PacketDef {
    fn format_into(self, tokens: &mut Tokens<Java>) {
        quote_in!(*tokens =>
            public class $(&self.name) {
                ...
                private $(&self.name)(Builder builder) {
                    $(for member in self.members.iter() => $(&member.name) = builder.$(&member.name);)
                }

                public static $(&self.name) fromBytes(byte[] bytes) {
