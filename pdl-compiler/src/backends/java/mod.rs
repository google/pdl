...
pub fn generate(
    ...
    let java_files = generate_java_files(dir, package, &file);
    java_files.iter().try_for_each(|f| f.generate(source.name()))
}

fn generate_java_files(output_dir: PathBuf, package: &str, file: &ast::File) -> Vec<JavaFile> {
...
fn write_tokens_to_file(path: PathBuf, package: &str, tokens: java::Tokens) -> Result<(), String> {
...
impl JavaFile {
    pub fn generate(&self, pdl_file_name: &str) -> Result<(), String> {
        ...
        write_tokens_to_file(self.get_path(), &self.package, tokens)
    }

    pub fn get_path(&self) -> PathBuf {
        // TODO(jmes): Make class name accessible without destructuring the 'def'
        let file_name = match &self.class {
            Class::Packet(PacketDef { name, .. }) => name,
            _ => todo!(),
        };
        self.dir.join(file_name).with_extension("java")
    }
}
