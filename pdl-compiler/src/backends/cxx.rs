...
impl<'a> FieldParser<'a> {
    ...
    fn parse_bit_field(&mut self, field: &'a ast::Field) {
        ...
        let should_skip_value =
            self.chunk.iter().all(|(_, _, f)| matches!(f.desc, ast::FieldDesc::Reserved { .. }));
        ...
        for (shift, width, field) in chunk.iter() {
            ...
            match &field.desc {
                ...
                ast::FieldDesc::Size { field_id, .. } => {
                    ...
                    self.unchecked_append(format!(
                        "{}{}_size_ = {};",
                        self.target_prefix, field_name, v
                    ));
                }
                ast::FieldDesc::Count { field_id, .. } => {
                    self.unchecked_append(format!(
                        "{}{}_count_ = {};",
                        self.target_prefix, field_id, v
                    ));
                }
        ...
        for f in self.decl.fields() {
            if found {
                match self.schema.field_size(f.key) {
                    ...
                    _ => panic!(
                        "Multiple unknown size fields in {}",
                        self.decl.id().unwrap_or("unknown")
                    ),
                }
            }
            if f.key == field.key {
    ...
    fn parse_payload_field_lite(&mut self, _field: &'a ast::Field, _is_body: bool) {
        ...
        if !size_expr.is_empty() {
            ...
            self.append(format!(
                "{}payload_ = span.subrange(0, {});",
                self.target_prefix, actual_size
            ));
    ...
    fn parse(&mut self, field: &'a ast::Field) {
        if field.cond.is_some() {
            ...
            match &field.desc {
                ...
                ast::FieldDesc::Payload { .. } | ast::FieldDesc::Body => {
                    self.parse_payload_field_lite(
                        field,
                        matches!(field.desc, ast::FieldDesc::Body),
                    );
                }
                ast::FieldDesc::Scalar { id, width } => {
...
impl<'a> FieldSerializer<'a> {
    ...
    fn get_payload_field_size(
        ...
        let get_field_size_expr = |f: &ast::Field| -> String {
            let fid = f.id();
            let is_constrained =
                fid.map(|fid| parent_constraints.iter().any(|c| c.id == fid)).unwrap_or(false);
            if is_constrained {
            ...
            match &f.desc {
                ast::FieldDesc::Scalar { width, .. } => {
                    if let Some(cond) = &f.cond {
                        let cond_field = self
                            .scope
                            .iter_fields(decl)
                            .find(|f| f.id() == Some(&cond.id))
                            .expect("Cond field not found");
                        let _cond_field_var =
                            format!("{0}.has_value() ? ...", cond_field.id().unwrap()); // This is getting complex
                ...
                ast::FieldDesc::Array { id, width, type_id, .. } => {
                    ...
                    if element_size.contains("GetSize") {
                        format!(
                            "std::accumulate({0}_.begin(), {0}_.end(), static_cast<size_t>(0), [](size_t s, auto const& element) {{ return s + {1}; }})",
                            id, element_size
                        )
                    } else {
        ...
        if field_id == "_payload_" || field_id == "_body_" {
            ...
            let size_expr = if has_local_payload {
                ...
                if local_fields.is_empty() { "0".to_string() } else { local_fields.join(" + ") }
            };
        ...
        let f =
            self.scope.iter_fields(decl).find(|f| f.id() == Some(field_id)).unwrap_or_else(|| {
                panic!("Field {} not found in {}", field_id, decl.id().unwrap_or("unknown"))
            });
        match &f.desc {
            ast::FieldDesc::Array { .. } => {
            ...
            match &field.desc {
                ...
                ast::FieldDesc::Typedef { type_id, .. } => {
                    ...
                    if let ast::DeclDesc::Enum { width, .. } = &td.desc {
                        let ty = get_cxx_scalar_type(*width);
                        self.append(&format!(
                            "pdl::packet::Builder::write_{}<{}, {}>(output, static_cast<{}>(*{}));",
                            byteorder,
                            ty,
                            width / 8,
                            ty,
            ...
            match &field.desc {
                ...
                ast::FieldDesc::Array { id, type_id, width, .. } => {
                    ...
                    if let Some(ps) = padded_size {
                        let ps_bytes = ps / 8;
                        self.append(&format!(
                            "if (output.size() - {0}_start < {1}) {{",
                            id, ps_bytes
                        ));
                        self.append(&format!(
                            "    output.resize({0}_start + {1}, 0);",
                            id, ps_bytes
                        ));
                        self.append("}");
                    }
                ...
                ast::FieldDesc::FixedScalar { width, value } => {
                    let ty = get_cxx_scalar_type(*width);
                    self.append(&format!(
                        "pdl::packet::Builder::write_{}<{}, {}>(output, {:#x});",
                        byteorder,
                        ty,
                        width / 8,
                        value
                ...
                ast::FieldDesc::Payload { .. } | ast::FieldDesc::Body => {
                    self.append(&format!(
                        "output.insert(output.end(), {0}.begin(), {0}.end());",
                        deref(var, "payload_")
                    ));
                }
                _ => {}
...
fn generate_packet_view(
    ...
    let parent_class =
        parent_id.map(|p| format!("{}View", p)).unwrap_or_else(|| "pdl::packet::slice".to_string());
    ...
    let all_fields =
        get_all_parent_fields(scope, decl).into_iter().chain(decl.fields()).collect::<Vec<_>>();

    let parent_constraints =
        scope.iter_parents_and_self(decl).flat_map(|d| d.constraints()).collect::<Vec<_>>();
        ...
        match &field.desc {
            ...
            ast::FieldDesc::Array { id, type_id, width, size, .. } => {
                ...
                let element_type = type_id
                    .as_deref()
                    .map(|t| t.to_string())
                    .unwrap_or_else(|| get_cxx_scalar_type(width.unwrap()));
                ...
                if let Some(s) = size {
                    ...
                    if let Some(tid) = type_id {
                        ...
                        match &td.desc {
                            ...
                            _ => {
                                accessor_code
                                    .push(format!("    {}::Parse(span, &elements[n]);", tid));
                            }
                        }
                    } else {
                        let element_width = width.unwrap();
                    ...
                    if let Some(tid) = type_id {
                        ...
                        match &td.desc {
                            ...
                            _ => {
                                ...
                                accessor_code.push(format!(
                                    "    if (!{}::Parse(span, &element)) break;",
                                    tid
                                ));
                                accessor_code.push(
                                    "    elements.emplace_back(std::move(element));".to_string(),
                                );
                                accessor_code.push("}".to_string());
                            }
                        }
            ...
            ast::FieldDesc::Scalar { id, width } => {
                ...
                if field.cond.is_some() {
                    ...
                    field_accessors.push(format!(
                        "    {} Get{}() const {{ _ASSERT_VALID(valid_); return {}_; }}\n",
                        ty, accessor_name, id
                    ));
                }
            }
...
fn generate_packet_builder(
    ...
    for f in &all_fields {
        ...
        match &f.desc {
            ast::FieldDesc::Scalar { width, .. } => {
                if let Some(cond) = &f.cond {
                    let cond_field = scope
                        .iter_fields(decl)
                        .find(|f| f.id() == Some(&cond.id))
                        .expect("Cond field not found");
                    let cond_field_var = get_field_expr(cond_field);
                    variable_widths.push(format!(
                        "(({} == {}) ? {} : 0)",
                        cond_field_var,
                        cond.value.unwrap(),
                        width / 8
            ...
            ast::FieldDesc::Array { id, width, type_id, .. } => {
                ...
                let array_size = if let Some(w) = width {
                    ...
                    match &td.desc {
                        ast::DeclDesc::Enum { width, .. } => {
                            format!("({0}_.size() * {1})", id, width / 8)
                        }
                        _ => format!(
                            "std::accumulate({0}_.begin(), {0}_.end(), static_cast<size_t>(0), [](size_t s, auto const& e) {{ return s + e.GetSize(); }})",
                            id
                        ),
                    }
                } else {
